...
- 🎯 Entry points to execute your program using `python -m <troyrc_ota>` or `$ troyrc_ota` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/ai-spirit/troyrc-ota/branch/main/graph/badge.svg?token=troyrc-ota_token_here)](https://codecov.io/gh/ai-spirit/troyrc-ota)
[![CI](https://github.com/ai-spirit/troyrc-ota/actions/workflows/main.yml/badge.svg)](https://github.com/ai-spirit/troyrc-ota/actions/workflows/main.yml)

Awesome troyrc_ota created by ai-spirit

## Install

from source
```bash
git clone https://github.com/ai-spirit/troyrc-ota troyrc_ota
cd troyrc_ota
make install
```
...
```bash
pip install troyrc_ota
```

## Executing

```bash
$ troyrc_ota run --port 8080
```

or

```bash
python -m troyrc_ota run --port 8080
```

or

```bash
$ uvicorn troyrc_ota:app
```
...
```bash
❯ troyrc_ota --help
Usage: troyrc_ota [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ troyrc_ota create-user --help
Usage: troyrc_ota create-user [OPTIONS] USERNAME PASSWORD
...
```bash
troyrc_ota create-user admin admin --superuser
```
...
Run with `troyrc_ota run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `troyrc_ota create-user`.
...
-----------------------------------------------------
troyrc_ota/__init__.py              4      0   100%
troyrc_ota/app.py                  16      1    94%
troyrc_ota/cli.py                  21      0   100%
troyrc_ota/config.py                5      0   100%
troyrc_ota/db.py                   10      0   100%
troyrc_ota/models/__init__.py       0      0   100%
troyrc_ota/models/content.py       47      1    98%
troyrc_ota/routes/__init__.py      11      0   100%
troyrc_ota/routes/content.py       52     25    52%
troyrc_ota/routes/security.py      15      1    93%
troyrc_ota/routes/user.py          52     26    50%
troyrc_ota/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in troyrc_ota/default.toml.
